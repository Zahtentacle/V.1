...
const App: React.FC = () => {
  ...
  const handleImport = () => {
    ...
    setTimeout(() => {
      ...
      addLog('SYSTEM', `Imported ${newContacts.length} targets.`, 'success');
    }, 1000);
  };
  ...
  useEffect(() => {
    ...
    const processNext = async () => {
      ...
      if (nextIndex === -1) {
        ...
        addLog('GAMMA', 'Queue cleared.', 'success');
        return;
      }
    ...
    <div className="min-h-screen bg-black text-zinc-100 flex font-sans overflow-hidden">
      ...
      <div className="w-64 border-r border-zinc-800 bg-zinc-950 flex flex-col fixed h-full z-10">
        ...
        <div className="p-4 border-t border-zinc-800">
            {!waConnected && (
              <button onClick={handleConnectWA} className="w-full bg-green-600 py-2 rounded text-xs font-bold text-white"><QrCode size={14} className="inline mr-2"/>LINK_WA</button>
            )}
        </div>
      </div>
      ...
      <main className="flex-1 ml-64 p-8 h-screen overflow-y-auto">
        <header className="flex justify-between items-center mb-8">
            ...
            <button onClick={toggleWorker} className={`px-8 py-3 rounded-lg font-black text-xs ${state.workerStatus === WorkerStatus.RUNNING ? 'bg-red-600' : 'bg-green-600'}`}>
                {state.workerStatus === WorkerStatus.RUNNING ? 'STOP ENGINE' : 'START ENGINE'}
            </button>
        </header>
        ...
        {activeTab === 'dashboard' && (
            <div className="space-y-6">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
                    <div className="lg:col-span-8"><TerminalLog logs={state.logs} /></div>
                    <div className="lg:col-span-4 bg-zinc-900 p-4 rounded-xl">
                        <h3 className="text-xs font-bold text-zinc-500 mb-4 uppercase">Workers</h3>
                        {state.workers.map(w => (
                            <div key={w.id} className="mb-4">
                                <div className="flex justify-between text-[10px] mb-1"><span>{w.name}</span><span>{w.load}%</span></div>
                                <div className="h-1 bg-zinc-800 rounded-full"><div className="h-full bg-green-500" style={{width: `${w.load}%`}}></div></div>
                            </div>
                        ))}
                    </div>
                </div>
        ...
        {activeTab === 'campaign' && (
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                <div className="bg-zinc-900 p-6 rounded-xl">
                    <h3 className="mb-4 font-bold">Message Template</h3>
                    <textarea 
                        className="w-full bg-black p-4 rounded border border-zinc-800 h-40 text-sm"
                        value={messageTemplate}
                        onChange={(e) => setMessageTemplate(e.target.value)}
                    />
                </div>
                <div className="bg-zinc-900 p-6 rounded-xl">
                    <h3 className="mb-4 font-bold">Import Contacts (Name, Phone)</h3>
                    <textarea 
                        className="w-full bg-black p-4 rounded border border-zinc-800 h-40 text-sm"
                        placeholder="Budi, 62812345678"
                        value={importText}
                        onChange={(e) => setImportText(e.target.value)}
                    />
                    <button onClick={handleImport} className="mt-4 bg-zinc-800 px-4 py-2 rounded text-xs w-full">IMPORT</button>
                </div>
            </div>
        )}
      </main>
